...
export function YouTubeEmbed({ videoId, title, className }: { videoId: string; title: string; className?: string }) {
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden
        className="absolute top-1/2 left-1/2 flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-paper text-ink shadow-lg transition-transform duration-200 group-hover:scale-105"
      >
